...
export function ArrDisplay({ state }: { state: State | undefined }) {
  ...
  function mapMainArray(arr: number[][]) {
    return (
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex gap-4 justify-center">
        {
          arr.map((arr) => {
            if (arr.length !== 0) {
              return (
  ...
  function mapSubArray(arr: number[]) {
    return (
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex justify-center border-2 rounded-2xl bg-amber-200 p-2 gap-4">
        {arr.map((num) => {
          return (
  ...
  if (!state) {
    ...
    return (
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex flex-col items-center text-center gap-6 p-4">
